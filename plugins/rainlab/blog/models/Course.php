...
class Course extends Model
{
    ...
    public $rules = [
        'title'   => 'required',
        'slug'    => ['required', 'regex:/^[a-z0-9\/\:_\-\*\[\]\+\?\|]*$/i', 'unique:rainlab_blog_courses'],
        'content' => 'required',
        'excerpt' => ''
    ];
    ...
    /**
     * The attributes on which the course list can be ordered.
     * @var array
     */
    ...
    public function afterValidate()
    {
        if ($this->published && !$this->published_at) {
            throw new ValidationException([
               'published_at' => Lang::get('rainlab.blog::lang.course.published_validation')
            ]);
        }
    }
    ...
    /**
     * Used to test if a certain user has permission to edit course,
     * returns TRUE if the user is the owner or has other courses access.
     * @param  User $user
     * @return bool
     */
    public function canEdit(User $user)
    {
        return ($this->user_id == $user->id) || $user->hasAnyAccess(['rainlab.blog.access_other_courses']);
    }
    ...
    /**
     * Lists courses for the frontend
     *
     * @param        $query
    ...
    public function scopeListFrontEnd($query, $options)
    {
         ...
         */
        if ($exceptCourse) {
            $exceptCourses = (is_array($exceptCourse)) ? $exceptCourse : [$exceptCourse];
            $exceptCourseIds = [];
            $exceptCourseSlugs = [];

            foreach ($exceptCourses as $exceptCourse) {
                $exceptCourse = trim($exceptCourse);

                if (is_numeric($exceptCourse)) {
                    $exceptCourseIds[] = $exceptCourse;
                } else {
                    $exceptCourseSlugs[] = $exceptCourse;
                }
            }

            if (count($exceptCourseIds)) {
                $query->whereNotIn('id', $exceptCourseIds);
            }
            if (count($exceptCourseSlugs)) {
                $query->whereNotIn('slug', $exceptCourseSlugs);
            }
        }
    ...
    /**
     * Used by "has_summary", returns true if this course uses a summary (more tag).
     * @return boolean
     */
     ...
     *
     *     // Get the next course
     *     Course::applySibling()->first();
     *
     *     // Get the previous course
     *     Course::applySibling(-1)->first();
     *
     *     // Get the previous course, ordered by the ID attribute instead
     *     Course::applySibling(['direction' => -1, 'attribute' => 'id'])->first();
     *
     * @param       $query
     * @param array $options
     * @return
    ...
    /**
     * Returns the next course, if available.
     * @return self
     */
    ...
    /**
     * Returns the previous course, if available.
     * @return self
     */
    ...
    public static function getMenuTypeInfo($type)
    {
        ...
        if ($type == 'blog-course') {
            ...
            $courses = self::orderBy('title')->get();
            foreach ($courses as $course) {
                $references[$course->id] = $course->title;
            }
        ...
        if ($type == 'all-blog-courses') {
            $result = [
                'dynamicItems' => true
            ];
        }

        if ($type == 'category-blog-courses') {
            $references = [];
        ...
        if ($result) {
            ...
            foreach ($pages as $page) {
                if (!$page->hasComponent('blogCourse')) {
                    continue;
                }

                /*
                 * Component must use a categoryPage filter with a routing parameter and course slug
                 * eg: categoryPage = "{{ :somevalue }}", slug = "{{ :somevalue }}"
                 */
                $properties = $page->getComponentProperties('blogCourse');
    ...
    public static function resolveMenuItem($item, $url, $theme)
    {
        ...
        if ($item->type == 'blog-course') {
            ...
            $pageUrl = self::getCoursePageUrl($item->cmsPage, $category, $theme);
            if (!$pageUrl) {
                return;
            }
        ...
        elseif ($item->type == 'all-blog-courses') {
            $result = [
                'items' => []
            ];

            $courses = self::isPublished()
            ->orderBy('title')
            ->get();
            ...
            foreach ($courses as $course) {
                $courseItem = [
                    'title' => $course->title,
                    'url'   => self::getCoursePageUrl($item->cmsPage, $course, $theme),
                    'mtime' => $course->updated_at
                ];

                $courseItem['isActive'] = $courseItem['url'] == $url;

                $result['items'][] = $courseItem;
            }
        }
        elseif ($item->type == 'category-blog-courses') {
            if (!$item->reference || !$item->cmsPage) {
                return;
            }
    ...
    /**
     * Returns URL of a course page.
     *
     * @param $pageCode
     * @param $category
     * @param $theme
     */
    protected static function getCoursePageUrl($pageCode, $category, $theme)
    {
