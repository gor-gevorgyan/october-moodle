...
class Categories extends ComponentBase
{
    ...
    /**
     * Load all categories or, depending on the <displayEmpty> option, only those that have blog courses
     * @return mixed
     */
    protected function loadCategories()
    {
        $categories = BlogCategory::with('courses_count')->getNested();
        if (!$this->property('displayEmpty')) {
            $iterator = function ($categories) use (&$iterator) {
                return $categories->reject(function ($category) use (&$iterator) {
                    if ($category->getNestedCourseCount() == 0) {
                        return true;
                    }
